...
| 找回密码 | `http://127.0.0.1:21026/recovery-password` |
...
后续访问需要管理员密码。忘记密码时，配置台会在当前标签页进入 `/recovery-password`。点击“发送恢复验证码”后，系统会通过 NEXUS 密码恢复服务将六位字母数字验证码发送到已绑定的恢复邮箱。

验证码有效期为 5 分钟，单个验证码最多校验 5 次；同一邮箱 60 秒内只能请求一次。发送冷却由中央服务器保存：刷新找回密码页面、完成一次密码重置后再次进入找回页面，都将显示真实剩余时间，冷却结束前不能重复发送。

“单个验证码最多校验 5 次”表示同一封邮件中的六码验证码最多可提交验证 5 次。连续输入错误达到 5 次后，该验证码会失效，需要等待发送冷却结束后重新获取新的验证码。

验证码统一由 NEXUS 中央恢复服务管理。验证码哈希、五分钟有效期、单邮箱 60 秒发送间隔与校验次数均存放在中央 Redis 中；验证成功后立即删除，超时后由 Redis 自动过期。用户部署的主页无需配置邮箱或 Redis，只会将恢复请求与用户输入的验证码安全转发至中央服务。

成功发送后请检查收件箱；如未找到验证码，请检查垃圾箱。邮件正文会注明验证码有效期为 5 分钟。

已登录配置台时，可通过顶部“修改密码”更新管理员密码。新密码不能与旧密码相同；更新成功后，当前登录会话会退出，需要使用新密码重新登录。
...
| `recoveryScope` | `UUID` | 此部署实例与中央恢复服务通信时使用的私有标识，用于隔离不同用户的验证码记录。 |
| `schemaVersion` | `2` | 认证文件格式版本，不属于认证凭证。 |
...
```json
{
  ...
  "recoveryScope": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
}
```
...
| `data/auth.json` | 管理员密码哈希、恢复邮箱与中央恢复服务的实例标识。 |
