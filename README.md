...
验证码有效期为 10 分钟，单个验证码最多校验 5 次；同一邮箱 60 秒内只能请求一次。发送冷却由服务器保存：刷新找回密码页面、完成一次密码重置后再次进入找回页面，都将显示真实剩余时间，冷却结束前不能重复发送。

“单个验证码最多校验 5 次”表示同一封邮件中的六码验证码最多可提交验证 5 次。连续输入错误达到 5 次后，该验证码会失效，需要等待发送冷却结束后重新获取新的验证码。

验证码不会写入 `data/` 或 VPS 磁盘。主页容器只在内存中保存验证码哈希，10 分钟后自动清理；中央邮件服务只负责投递邮件与发送频率控制，不保存验证码内容。

成功发送后请检查收件箱；如未找到验证码，请检查垃圾箱。邮件正文会注明验证码有效期为 10 分钟。
