...
| 局域网主页 | `http://你的IP:8099/` |
| 局域网配置台 | `http://你的IP:8099/config` |
...
后续访问需要管理员密码。点击“使用邮箱验证码重置”后，系统会将六码字母数字验证码发送到已绑定邮箱；验证码有效期为 10 分钟，单个验证码最多校验 5 次。
...
docker run -d --name nexus-home --restart unless-stopped -p 21026:8099 -v nexus-home-data:/app/data yuni020126/nexus-home:latest
```
